...
void FdcCom(PCAPSFDC pc, UDWORD data)
{
	...
	if (comcl == 0xd)
		...
		// clear drq, intrq lines
		FdcSetLine(pc, pc->lineout&~(CAPSFDC_LO_DRQ|CAPSFDC_LO_INTRQ));
	}
}
...
void FdcComT2W(PCAPSFDC pc)
{
	...
	// !!!
	CString s;
	s.Format("Not implemented: %2.2x", pc->r_command);
	AfxMessageBox(s);
#endif
	pc->endrequest|=CAPSFDC_ER_COMEND;
}
...
void FdcComT4(PCAPSFDC pc)
{
	...
	if (pc->r_st0 & CAPSFDC_SR_BUSY) {
		...
		pc->r_stm=CAPSFDC_SM_TYPE1;
		pc->r_st0&=~(CAPSFDC_SR_RNF|CAPSFDC_SR_CRCERR);
		pc->r_st1=0;
	}

	// clear drq, index and forced interrupt
...
void FdcUpdateData(PCAPSFDC pc)
{
	...
	// keep state if track.side are the same with buffered data
