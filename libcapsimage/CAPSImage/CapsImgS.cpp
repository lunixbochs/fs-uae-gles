// CapsImgS.cpp: implementation of the CCapsImageStd class.
//
//////////////////////////////////////////////////////////////////////
...
int CCapsImageStd::LoadTrack(PDISKTRACKINFO pti, UDWORD flag)
{
	...
	// only update the track image if already encoded
	if (pti->trackcnt)
		return UpdateImage();
...
{
	CDWordArray data;
	data.SetSize(0, DEF_SCANINFO);
	CDWordArray trck;
	trck.SetSize(0, DEF_SCANINFO);
	DiskTrackInfo dt;
	...
	for (int run=1; run; ) {
		...
		if (did && did<trck.GetSize() && did<data.GetSize()) {
			if (trck[did] && data[did]) {
				// reset track info
				...
				dt.headerpos=trck[did];
				dt.datapos=data[did];

				// create data info
				loader.SetPosition(data[did]);
				loader.ReadChunk();
...
int CCapsImageStd::UpdateWeakBit()
{
	// don't change track data if updates are disabled
	if (di.flag & DI_LOCK_NOUPDATE)
		return imgeOk;
...
void CCapsImageStd::UpdateOverlap()
{
	...
	for (int trk=0; trk < di.pdt->trackcnt; trk++) {
		uint32_t value=ReadBit(bitpos, ovlsize);
		value=~value;
		WriteBit(bitpos, value, ovlsize);
...
int CCapsImageStd::ProcessImage()
{
	...
	int bufsize=GetByteSize(alltrackbits);
	uint8_t *tbuf;
...
int CCapsImageStd::GenerateCLA(PDISKTRACKINFO pti, PUBYTE buf)
{
	...
	int lg=GetByteSize(di.block[3].gapbits);
	int pos=pti->sdpos, cp;

	for (unsigned blk=0; blk < pti->ci.blkcnt; blk++) {
		int bs=GetByteSize(di.block[blk].blockbits);
		int gs=GetByteSize(di.block[blk].gapbits);
		int xs=bs+gs;
...
int CCapsImageStd::GenerateCLA2(PDISKTRACKINFO pti, PUBYTE buf)
{
	...
	// iterate block positions
	int lg=GetByteSize(di.block[di.blockcount-1].gapbits);
	int pos=pti->sdpos, cp;
...
int CCapsImageStd::GetBlock(PCAPSBLOCK pb, int blk)
{
	...
	CCapsLoader::Swap((uint32_t *)pb, sizeof(CapsBlock));
...
int CCapsImageStd::GetSampleGap(PIMAGESTREAMINFO pstr)
{
	...
	while (1) {
		...
		if (pstr->sizemodebit) {
			bitcount=count;
			bytecount=GetByteSize(bitcount);
		} else {
			bytecount=count;
...
// write bitfield to track buffer
void CCapsImageStd::WriteBit(uint32_t bitpos, uint32_t value, int bitcnt)
{
	// select slow write mode if track would wrap around during writing, otherwise use fast write
	if (bitpos+bitcnt > di.trackbc) {
		...
		uint8_t *wpos=di.track+(bitpos >> 3);
		...
		*wpos=(uint8_t)data;
		...
		// fast mode: buffer can't wrap, process in batches of bytes
...
// clear bits in track buffer
void CCapsImageStd::ClearBit(uint32_t bitpos, int bitcnt)
{
	while (bitcnt > 0) {
		// write largest possible bitfield
		int writebc=(bitcnt >= maxwritelen) ? maxwritelen : bitcnt;
...
// read up to 32 data bits
uint32_t CCapsImageStd::ReadBit(uint8_t *buf, uint32_t bitpos, int bitcnt)
{
	uint32_t res=0;
	uint32_t bit=1 << ((bitpos & 7) ^ 7);
	uint32_t pos=bitpos >> 3;
	uint32_t str=buf[pos++];
...
// read up to 32 data bits from track buffer
uint32_t CCapsImageStd::ReadBit(uint32_t bitpos, int bitcnt)
{
	uint32_t res;

	// select slow write mode if track would wrap around during reading, otherwise use fast read
	if (bitpos+bitcnt > di.trackbc) {
		res=0;
		...
		res=ReadBit(di.track, bitpos, bitcnt);
