...
static int bip_cdtv (struct uae_prefs *p, int config, int compa, int romcheck)
{
    ...
    // moved configure_rom down here, so other CDTV settings will be
    // configured as normal when the roms where not found (roms can be
    // configure later)
    roms[0] = 6;
    roms[1] = 32;
    roms[2] = -1;
...
static int bip_cd32 (struct uae_prefs *p, int config, int compa, int romcheck)
{
	...
	// moved configure_rom down here, so other CD32 settings will be
	// configured as normal when the roms where not found (roms can be
	// configure later)
    roms[0] = 64;
    roms[1] = -1;
