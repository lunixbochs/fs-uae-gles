...
static void fs_ml_signal_buffer_swap() {
...
void fs_ml_wait_for_buffer_swap_signal() {
...
void fs_ml_signal_buffer_swap_done() {
...
static void gl_finish() {
    // FIXME: Make use of glFinish configurable?
    int64_t t1 = fs_get_monotonic_time();
    fs_gl_finish();
...
void fs_ml_render_iteration() {
    ...
    if (g_fs_ml_vblank_sync) {
        // the same code is run whether full video sync is used or not
